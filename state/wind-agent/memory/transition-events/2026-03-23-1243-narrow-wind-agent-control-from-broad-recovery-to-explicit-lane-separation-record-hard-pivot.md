...
- state/wind-agent/current/current-task.md already distinguishes one proven query contract from a broader still-open convergence line.
- state/wind-agent/current/blockers.md records scope and validation drift that the broad objective does not separate cleanly enough.

...
paths:
  ...
  - state/repo-governance-kernel/control/
thread_ids: []
evidence_refs:
  - type: doc
  ...
  - type: note
    ref: C:/Users/terryzzb/Desktop/session-memory/state/repo-governance-kernel/current/current-task.md
tags:
  - objective
  - control-system
  - active
...
Phase changed from `paused` to `execution` because Retire stale legacy identities, remove legacy demo framing from the live product surface, and complete the repo-owned rename honestly.
